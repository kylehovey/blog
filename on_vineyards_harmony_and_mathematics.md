...
Once again, mathematics has a name for this process and the terms it generates: the [Farey Sequence](https://en.wikipedia.org/wiki/Farey_sequence){:target="_blank"}. The only difference is that the Farey Sequence does allow for repeated terms. This means that for the sixth order, the inclusion of \\(\frac{2}{6}\\) will be allowed and be written as \\(\frac{1}{3}\\). Each successive iteration of the Farey Series also includes the last, but adds the [mediant](https://en.wikipedia.org/wiki/Mediant_\(mathematics\){:target="_blank"}) between each pair. This yields a similar, but unique definition of "The \\(n\\)th Farey sequence contains all irreducible fractions less than \\(1\\) with denominators less than \\(n\\)".
