...
[Here is a live demo of the app](http://kylehovey.github.io/automata-nebula-explorer/index.html){:target="blank"}. Both the view of the full nebula and the zoomed portion are clickable, it just takes a second to find the closest rule. Please note that the page will take a few seconds to load as initializing the data for all quarter million rules is a processor-heavy task. As a result, I don't expect mobile performance to hold up (or even work). I'm open to PR's to improve the app.
